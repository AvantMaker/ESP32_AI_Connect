...
**Auto-retry is enabled by default!** No configuration needed—it works automatically.

### Optional: Customize Configuration

If you want to adjust the retry behavior, define the configuration values **before** including the library:

**Arduino IDE** - In your `.ino` file:
```cpp
// Customize auto-retry settings (optional)
...
**PlatformIO** - In your `platformio.ini`:
```ini
build_flags = 
    -DAUTO_RETRY_MAX_ATTEMPTS=3
    -DAUTO_RETRY_INITIAL_DELAY_MS=1000
    -DAUTO_RETRY_MAX_DELAY_MS=10000
    -DAUTO_RETRY_STALE_CONNECTION_THRESHOLD_MS=300000
```
...
**PlatformIO:**
```ini
build_flags = -DDISABLE_AUTO_RETRY
```
...
- **~400 bytes** of code (only when auto-retry is enabled)
- **Zero overhead** when feature is disabled via `DISABLE_AUTO_RETRY`
...
### 1. Keep Auto-Retry Enabled (Default)

Auto-retry is enabled by default and recommended for all production deployments. Only disable it if you have specific requirements.
...
**Solution:** Reduce retry attempts (define before including the library):
```cpp
...
**Solution:** Increase retry attempts (define before including the library):
```cpp
...
**Solution:** Increase threshold (define before including the library):
```cpp
...
- **Enabled by default** (can be disabled via `DISABLE_AUTO_RETRY`)
- **No API changes** required
- **Existing code works unchanged**
- **Zero overhead when disabled**

Users can disable the feature if needed, but it's recommended to keep it enabled for better reliability.
